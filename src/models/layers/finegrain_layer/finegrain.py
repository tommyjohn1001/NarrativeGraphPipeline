...
class FineGrain(torch_nn.Module):
    ...
    def __init__(self,
        ...
        self.biGRU_CoAttn = torch_nn.GRU(d_bert, d_bert//2, num_layers=n_gru_layers,
                                         batch_first=True, bidirectional=True)

        self.biGRU_attn = torch_nn.GRU(d_bert, d_bert//2, num_layers=n_gru_layers,
                                       batch_first=True, bidirectional=True)

        self.biGRU_mask = torch_nn.GRU(d_bert, d_bert//2, num_layers=n_gru_layers,
                                       batch_first=True, bidirectional=True)
    ...
    def encode_ques_para(self, ques, paras, ques_mask, paras_mask):
        ...
        for ith in range(n_paras):
            ...
            # Embed context
            ###################
            L_s = self.bert_emb(para, para_mask)
