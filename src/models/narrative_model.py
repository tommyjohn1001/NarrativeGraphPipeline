...
class NarrativeModel(plt.LightningModule):
    def __init__(self,
        ...
        # Define model
        #############################
        self.embd_layer = BertBasedLayer(d_vocab, d_bert, path_bert)
        self.reasoning  = MemoryBasedReasoning(seq_len_ques, seq_len_para,
                            seq_len_ans, n_paras, n_layers_gru, n_heads_trans,
                            n_layers_trans, d_hid, d_bert, self.device)
    ...
    def generate(self, decoder_input_ids, encoder_outputs):
        ...
        decoder_input_embd  = self.embd_layer.encode_ans(decoder_input_ids, decoder_input_mask)
        # [b_, seq=*, d_bert]
