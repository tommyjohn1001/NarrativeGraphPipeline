...
class NarrativeDataset(Dataset):
    ...
    def f_process_file_single(self, entry):
        ...
        # Process question
        ###########################
        encoded = self.tokenizer(entry.question, padding='max_length', max_length=self.seq_len_ques,
        ...
        encoded     = self.tokenizer(ans1, padding='max_length', max_length=self.seq_len_ans,
                                     return_tensors='np', return_token_type_ids=False)
        ans1        = encoded['input_ids'][0]
        ans1_mask   = encoded['attention_mask'][0]

        encoded     = self.tokenizer(ans2, padding='max_length', max_length=self.seq_len_ans,
                                     return_tensors='np', return_token_type_ids=False)
        ...
        for ith, para in enumerate(contx):
            encoded         = self.tokenizer(para, padding='max_length', max_length=self.seq_len_para,
                                             return_tensors='np', return_token_type_ids=False)
    ...
    def read_datasetfile(self, path_file):
        ...
        for entry in entries:
            ...
            self.ans2_mask.append(entry['ans1_mask'])
